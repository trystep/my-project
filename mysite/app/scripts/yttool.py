...
class LivechatReader:
    ...
    def getchatinfo(self, cfg):
        ...
        """
        item = getitem(cfg, ("playerResponse",), "response", "contents", "twoColumnWatchNextResults", "conversationBar",
                       "liveChatRenderer")
        if not item:
            return
...
class CommentReader:
    ...
    def getcommentinfo(self, cfg):
        ...
        """
        item = getitem(cfg, ("response",), "response", "contents", "twoColumnWatchNextResults", "results", "results",
                       "contents")
...
class SearchReader:
    ...
    def getresults(self, js):
        ct = getitem(js, "contents", "twoColumnSearchResultsRenderer", "primaryContents", "sectionListRenderer",
                     "contents")
        if not ct:
        ...
        cont = getitem(ct, ("continuationItemRenderer",), "continuationItemRenderer", "continuationEndpoint",
        ...
        for i in range(1):
            ...
            for item in resultlist:
                if video := item.get("videoRenderer"):
                    ...
                    # ownerText -> runs
                    data.append([vid, self.extractruns(title)])
...
class DetailReader:
    ...
    def output(self):
        ...
        twocol = getitem(self.cfg, ("response",), "response", "contents", "twoColumnWatchNextResults", "results",
                         "results", "contents")
        sentiment = getitem(twocol, ("videoPrimaryInfoRenderer",), "videoPrimaryInfoRenderer", "sentimentBar",
        ...
        data = {'id': vid, 'title': title, 'owner': owner, 'view_count': vc, 'duration': ls, 'sentiment': sentiment,
                'publish_date': pd,
                'upload_date': ud, 'description': desc}
        return data
        ...
        if playlist:
            ...
            for entry in getitem(playlist, "playlist", "contents"):
                ...
                if args.verbose:
                    ...
                    print("%s - %s" % (vid, title))
            return
        tabs = getitem(self.cfg, ("response",), "response", "contents", "twoColumnBrowseResultsRenderer", "tabs", 0,
                       "tabRenderer", "content")
        playlist = getitem(tabs, "sectionListRenderer", "contents", 0, "itemSectionRenderer", "contents", 0,
            ...
            while cont:
                ...
                if playlist:
                    for entry in getitem(playlist, "items"):
                        ...
                        print("%s - %s" % (vid, title))
                playlist = getitem(js, ("response",), "response", "continuationContents",
